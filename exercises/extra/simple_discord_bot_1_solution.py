...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        print(f"{guild_name} - {guild_id} - Members: {member_count}")

        member_sum = member_sum + member_count
...
async def greet(interaction: discord.Interaction, member: discord.Member):
    ...
    await interaction.response.send_message(f"Hello {member.display_name}")  # multiple options are possible for that!
...
async def add(interaction: discord.Interaction, member: discord.Member, channel: discord.TextChannel):
    """ Greet a user with the bot"""
